...
# FIELD_CRAFTER_MEMORY_STRUCTURAL_RECOVERY_V7_1_TEST
...
def _drift_profile(
    ...
    *,
    ...
    return MemoryProfile(
        ...
        source=f"{original.source}+v7.1-self-test-{'-'.join(tags)}",
        data=data,
    )
...
def _run_scenario(
    *,
    ...
    drifted = _drift_profile(
        ...
        object_graph_drift=object_graph_drift,
    ...
    return {
        ...
        "intentional_drift": {
            ...
            "object_graph": object_graph_drift,
            "entry_layout": entry_drift,
        },
        "pre_recovery_proofs": {
...
def main() -> int:
    ...
    try:
        ...
        if (
            ...
            or float(policy.get("minimum_roster_entity_score_margin") or 0) < 5.0
            or float(policy.get("minimum_entity_character_score_margin") or 0) < 4.0
            or float(policy.get("minimum_entry_pair_score_margin") or 0) < 5.0
            or float(policy.get("minimum_joint_hypothesis_margin") or 0) < 5.0
        ):
            raise RuntimeError(
