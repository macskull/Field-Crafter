...
def main() -> int:
    ...
    try:
        ...
        s=run_step(
            'Build all-wheel offline runtime wheelhouse',
            [str(rp),'-m','pip','download','--only-binary=:all:','--dest',str(wheelhouse),
        ...
        output=diagnostics_dir()/f'field_crafter_release_environment_test_{stamp}.json'
        try:
