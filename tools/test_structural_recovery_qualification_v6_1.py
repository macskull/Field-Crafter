...
def _extract_sample_metrics(
    ...
    for kind, block, prefix in (
        ...
        metrics[f"{prefix}_header"] = {
            ...
            "candidate_count": len(
                (
                    char_candidate.get("raw_inventory_headers", {})
                    .get(kind, {})
                    .get("populated_candidates", [])
                )
                if char_candidate else []
            ),
            "unique_candidate": False,
            "expected_is_best_scoring": (
                bool(block.get("available"))
                and int(header.get("delta_from_expected") or 0)
                == header_expected
            ),
            "clear_winner": False,
            "expected_is_clear_winner": False,
            "expected_score": None,
            "best_score": None,
...
def main() -> int:
    ...
    try:
        ...
        result = {
            "passed": True,
            "qualification_harness_version": "6.1",
            "profile_id": original.profile_id,
            "profile_version": original.profile_version,
