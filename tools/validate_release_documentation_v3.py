...
def main() -> int:
    ...
    try:
        ...
        paths = {
            ...
            "direct_player_test": root / "tools" / "test_direct_player_inventory_v8.py",
            "memory_schema_test": root / "tools" / "test_memory_profile_schema_v2.py",
        }
        ...
        require(
            ...
            "GAME MEMORY RESOLUTION" in notes
            and "INVENTORY LAYOUTS" in notes
            and "APPLICATION UPDATES" in notes
            and "MEMORY DEFINITION UPDATES" in notes,
            "1.16.2 release notes must cover the resolver, inventory layouts, and both update channels.",
        )
        ...
        require(
            ...
            and release_meta.get("name") == f"Field Crafter {version}"
            and release_meta.get("prerelease") is False
            and release_meta.get("notes_file") == f"GITHUB_RELEASE_{version}.md",
            "GitHub release metadata must target the current release.",
        )
        require(
            checks,
        ...
        require(
            ...
            release_body.startswith(f"# Field Crafter {version}")
        ...
        require(
            ...
            "direct local-player" in release_body.lower()
            and "inventory" in release_body.lower()
            and "application-update" in release_body.lower(),
            "Prepared GitHub release body must explain the resolver/layout hotfix and updater path.",
        )
        ...
        require(
            checks,
            "build_uses_documentation_v3",
            "tools\\validate_release_documentation_v3.py" in build,
            "build_release.ps1 must use the 1.16.2-aware documentation validator.",
        )
        require(
            checks,
        ...
        require(
            checks,
            "build_runs_direct_player_regression",
            "tools\\test_direct_player_inventory_v8.py" in build,
            "Release packaging must run the direct-player/multi-layout regression suite.",
        )
        require(
            checks,
            "build_runs_memory_schema_regression",
            "tools\\test_memory_profile_schema_v2.py" in build,
            "Release packaging must run the memory-profile schema-v2 regression suite.",
        )
        require(
            checks,
            "build_runs_privacy_regression",
            "tools\\test_privacy_redaction_v1.py" in build,
            "Release packaging must run the UI/diagnostic privacy-redaction regression suite.",
        )
        require(
            checks,
